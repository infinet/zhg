...
1.005564,1.114255,0.902073,0.868974,0.95253,1.041639,1.982235,1.244247,0.992191,1.865165
  ...
  for (long i=0; i< encal; ++i)
 {
    ...
    if(n%2 == 0)
    {
      scaledcal[l][m][n] = 0.95*corrector[l][m]*(factorzhangmodified[l][m][n]/(factorincrease[oddposition]/1.147697))*truncatedsinglecal[l][m][n];
    }else
    {
      scaledcal[l][m][n] = 0.95*corrector[l][m]*(factorzhangmodified[l][m][n]/(factordecrease[oddposition]/1.11879))*truncatedsinglecal[l][m][n];
    }
    ...
    if(n%2 == 0)
     {
      scaledcal[l][m][n] = 0.95*corrector[l][m]*(factorzhangmodified[l][m][n]/(factordecrease[evenposition]/1.11879))*truncatedsinglecal[l][m][n];
     }else
     {
      scaledcal[l][m][n] = 0.95*corrector[l][m]*(factorzhangmodified[l][m][n]/(factorincrease[evenposition]/1.147697))*truncatedsinglecal[l][m][n];
    }
    }
    }
    }
