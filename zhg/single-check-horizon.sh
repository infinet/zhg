...
double total[50]={0.926986535576579,0.937054712229145,0.937054712229145,0.937054712229145,0.937054712229145,0.964600893948624,0.993885102003424,1.00080044174796,0.994912120020779,0.979954210389821,1.01065841374392,1.00720007591732,1.01374775154554,1.00466874848418,1.00185826264828,0.99907673691451,1.01051384358765,1.00647190055387,1.01186161647155,1.02426048849325,1.03882305976484,1.00533247011857,1,1.01317702450239,1.0373269772729,1.01076016285056,1.03211943571219,0.986200043384083,1.01695611547302,1.02032034963268,1.00760893061852,0.994506760348587,1.02226164473744,1.02309675171253,1.01466113233257,1.00613930311762,1.02016256644824,0.97820549125076,1.01101248080226,1.02202466439399,0.997675189001405,0.99026988850813,1.00314799129029,1.00304988834442,0.959412145499695,0.989292785866185,0.991376054578266,0.968470734511869,0.97479800055564,1.01673074529857};
...
/*
  ...
  for(int n=0; n <50; n++)
   {
     scaledcal[l][m][n] = 0.95*corrector[l][m]*total[n]*horizoncorrector1[n]*horizoncorrector2[n]*factorzhangmodified[l][m][n]*truncatedsinglecal[l][m][n] ;
   }
   }
   }
  ...
  for(int n=0; n <50; n++)
   {
     scaledcal[l][m][n] = 0.95*corrector[l][m]*total[n]*verticalcorrector1[n]*verticalcorrector2[n]*factorzhangmodified[l][m][n]*truncatedsinglecal[l][m][n] ;
   }
   }
   }
...
*/
    ...
    for(int n=0; n <50; n++)
    {
      if(n%2 == 0)
      {
      scaledcal[l][m][n] = s*corrector[l][m]*total[n]*horizoncorrector1[n]*horizoncorrector2[n]*(factorzhangmodified[l][m][n]*factorincrease[oddposition]/factorincrease[newoddposition])*truncatedsinglecal[l][m][n];
      }else
      {
      scaledcal[l][m][n] = s*corrector[l][m]*total[n]*horizoncorrector1[n]*horizoncorrector2[n]*(factorzhangmodified[l][m][n]*factordecrease[oddposition]/factordecrease[newoddposition])*truncatedsinglecal[l][m][n];
      }
    ...
    if(n%2 == 0)
     {
      scaledcal[l][m][n] = s*corrector[l][m]*total[n]*verticalcorrector1[n]*verticalcorrector2[n]*(factorzhangmodified[l][m][n]*factorincrease[evenposition]/factorincrease[newevenposition])*truncatedsinglecal[l][m][n];
     }else
     {
      scaledcal[l][m][n] = s*corrector[l][m]*total[n]*verticalcorrector1[n]*verticalcorrector2[n]*(factorzhangmodified[l][m][n]*factordecrease[evenposition]/factordecrease[newevenposition])*truncatedsinglecal[l][m][n];
      }
...
*/
   ...
   for(int n=0;n <50; n++)
   {
     fivesumcal[l][m][n] =  scaledcal[l][m][n] ;
   }
