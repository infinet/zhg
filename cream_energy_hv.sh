...
#include <stdio.h>
  ...
  for (long i=0; i< encal; ++i)
 {
   ...
   for(int n=0;n <1; n++)
   {
     ...
     sumcal[l][m][n] =  shiftedcal[l][m][n] +  shiftedcal[l][m][n+1] +  shiftedcal[l][m][n+2] +  shiftedcal[l][m][n+3] +  shiftedcal[l][m][n+4];
   }
   ...
   for(int n=1;n <2; n++)
    {
     sumcal[l][m][n] =  shiftedcal[l][m][n-1]  +  shiftedcal[l][m][n] + shiftedcal[l][m][n+1] + shiftedcal[l][m][n+2]+ shiftedcal[l][m][n+3];
    }

   for(int n=48;n <49; n++)
    {
     sumcal[l][m][n] =  shiftedcal[l][m][n-3] +shiftedcal[l][m][n-2] +  shiftedcal[l][m][n-1] +  shiftedcal[l][m][n] + shiftedcal[l][m][n+1];
    }


   for(int n=49;n <50; n++)
    {
      sumcal[l][m][n] = shiftedcal[l][m][n-4]  +  shiftedcal[l][m][n-3]  +shiftedcal[l][m][n-2]  +  shiftedcal[l][m][n-1]  +  shiftedcal[l][m][n];
    }
...
double signaldecrease[50]={1,0.994911,0.989848,0.98481,0.979799,0.974812,0.969852,0.964916,0.960005,0.95512,0.950259,0.945423,0.940612,0.935825,0.931063,0.926325,0.92161,0.91692,0.912254,0.907612,0.902993,0.898397,0.893825,0.889277,0.884751,0.880249,0.875769,0.871312,0.866878,0.862466,0.858077,0.85371,0.849366,0.845043,0.840743,0.836464,0.832208,0.827972,0.823759,0.819567,0.815396,0.811246,0.807118,0.80301,0.798924,0.794858,0.790813,0.786788,0.782784,0.778801};

double signalincrease[50]={0.778801,0.782784,0.786788,0.790813,0.794858,0.798924,0.80301,0.807118,0.811246,0.815396,0.819567,0.823759,0.827972,0.832208,0.836464,0.840743,0.845043,0.849366,0.85371,0.858077,0.862466,0.866878,0.871312,0.875769,0.880249,0.884751,0.889277,0.893825,0.898397,0.902993,0.907612,0.912254,0.91692,0.92161,0.926325,0.931063,0.935825,0.940612,0.945423,0.950259,0.95512,0.960005,0.964916,0.969852,0.974812,0.979799,0.98481,0.989848,0.994911,1};


double factordecrease[50]={1.13000479122031,1.12374456467695,1.11751823827733,1.11132694202623,1.10517067592367,1.09904717996004,1.09295871414495,1.08690414847359,1.08088235294118,1.07489332754771,1.06893820229798,1.06301697719198,1.05712739222014,1.05127057738725,1.0454465326933,1.0396541281335,1.03389449371265,1.02816649942596,1.02247014527342,1.01680543125503,1.0111723573708,1.00557092362072,1,0.994459586508647,0.988949683146656,0.98347141991882,0.978022536815556,0.972604163841655,0.967216300997116,0.961856688272358,0.956528715681754,0.951228993210931,0.94595865086468,0.940718818647791,0.935506106545892,0.930323904573355,0.9251699527206,0.920044250987624,0.914946799374429,0.909877597881015,0.904836646507381,0.899823945253528,0.894839494119455,0.88988103309558,0.884950822191486,0.880048861407172,0.875172890733057,0.87032404017393,0.865502309729793,0.860707699400645};


double factorincrease[50]={0.884951690789436,0.889881353345505,0.894838899922631,0.899824330520815,0.904837645140055,0.909877681946143,0.914946764607498,0.920044893124121,0.92516974382759,0.930323640386328,0.935506582800333,0.940718571069607,0.945959605194148,0.951229685173957,0.956528811009034,0.961856982699379,0.967216523913413,0.972603949148504,0.978022743907285,0.983471746355544,0.988949794659072,0.994460374320499,0.999999999837194,1.00557099487758,1.01117335944165,1.01680593169521,1.02247103530666,1.0281675084418,1.03389535110063,1.03965456328315,1.04544630682357,1.05127058172189,1.05712738797811,1.06301672559224,1.06893859456426,1.07489415672839,1.08088225025042,1.08690403696456,1.09295951687082,1.09904752813497,1.10517039442544,1.11132811574224,1.11751953025115,1.12374463795216,1.1300046006795,1.13630058026738,1.14263025304736,1.14899594268787,1.15539764918892,1.16183421071629};
...
/*
    ...
    for(int n=0; n <50; n++)
    {
      if(n%2 == 0)
      {
      scaledcal[l][m][n] = s*corrector[l][m]*total[n]*horizoncorrector[n]*factorzhangmodified[l][m][n]*truncatedsinglecal[l][m][n];
      }else
      {
      scaledcal[l][m][n] = s*corrector[l][m]*total[n]*horizoncorrector[n]*factorzhangmodified[l][m][n]*truncatedsinglecal[l][m][n];
      }
    ...
    if(n%2 == 0)
     {
      scaledcal[l][m][n] = s*corrector[l][m]*total[n]*verticalcorrector[n]*factorzhangmodified[l][m][n]*truncatedsinglecal[l][m][n];
     }else
     {
      scaledcal[l][m][n] = s*corrector[l][m]*total[n]*verticalcorrector[n]*factorzhangmodified[l][m][n]*truncatedsinglecal[l][m][n];
      }
...
*/
    ...
    for(int n=0; n <50; n++)
    {
      if(n%2 == 0)
      {
      scaledcal[l][m][n] = s*total[n]*corrector[l][m]*horizoncorrector[n]*(factorzhangmodified[l][m][n]*signalincrease[oddposition]/signalincrease[newoddposition])*truncatedsinglecal[l][m][n];
      }else
      {
      scaledcal[l][m][n] = s*total[n]*corrector[l][m]*horizoncorrector[n]*(factorzhangmodified[l][m][n]*signaldecrease[oddposition]/signaldecrease[newoddposition])*truncatedsinglecal[l][m][n];
      }
    ...
    if(n%2 == 0)
     {
      scaledcal[l][m][n] = s*total[n]*corrector[l][m]*verticalcorrector[n]*(factorzhangmodified[l][m][n]*signaldecrease[evenposition]/signaldecrease[newevenposition])*truncatedsinglecal[l][m][n];
     }else
     {
      scaledcal[l][m][n] = s*total[n]*corrector[l][m]*verticalcorrector[n]*(factorzhangmodified[l][m][n]*signalincrease[evenposition]/signalincrease[newevenposition])*truncatedsinglecal[l][m][n];
      }
...
#include <stdio.h>
  ...
  for (long i=0; i< encal; ++i)
 {
   ...
   for(int n=0;n <1; n++)
   {
     ...
     int num2 = l*50*10 + m*50 + n+1;
     int num3 = l*50*10 + m*50 + n+2;
     fivesumcal[l][m][n] =  scaledcal[l][m][n] +  scaledcal[l][m][n+1] +  scaledcal[l][m][n+2]+  scaledcal[l][m][n+3] + scaledcal[l][m][n+4];
   }
   ...
   for(int n=1;n <2; n++)
    {
     fivesumcal[l][m][n] =  scaledcal[l][m][n-1]  +  scaledcal[l][m][n] + scaledcal[l][m][n+1] + scaledcal[l][m][n+2] + scaledcal[l][m][n+3];
    }

   for(int n=48;n <49; n++)
    {
     fivesumcal[l][m][n] = scaledcal[l][m][n-3] +  scaledcal[l][m][n-2] +  scaledcal[l][m][n-1] +  scaledcal[l][m][n] + scaledcal[l][m][n+1];
    }


   for(int n=49;n <50; n++)
    {
      fivesumcal[l][m][n] = scaledcal[l][m][n-4] + scaledcal[l][m][n-3] + scaledcal[l][m][n-2]  +  scaledcal[l][m][n-1]  +  scaledcal[l][m][n];
    }
