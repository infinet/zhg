...
*/
    ...
    for(int n=0; n <50; n++)
    {
      if(n%2 == 0)
      {
      scaledcal[l][m][n] = s*corrector[l][m]*horizoncorrector[n]*(factorzhangmodified[l][m][n]*signaloddincrease[$even]/signaloddincrease[$ijob])*truncatedsinglecal[l][m][n];
      }else
      {
      scaledcal[l][m][n] = s*corrector[l][m]*horizoncorrector[n]*(factorzhangmodified[l][m][n]*signalodddecrease[$even]/signalodddecrease[$ijob])*truncatedsinglecal[l][m][n];
      }
    ...
    if(n%2 == 0)
     {
    scaledcal[l][m][n] = s*corrector[l][m]*verticalcorrector[n]*(factorzhangmodified[l][m][n]*signalevendecrease[$odd]/signalevendecrease[$newodd])*truncatedsinglecal[l][m][n];
     }else
     {
    scaledcal[l][m][n] = s*corrector[l][m]*verticalcorrector[n]*(factorzhangmodified[l][m][n]*signalevenincrease[$odd]/signalevenincrease[$newodd])*truncatedsinglecal[l][m][n];
      }
...
#include <stdio.h>
  ...
  for (long i=0; i< encal; ++i)
 {
   ...
   for(int m=0;m <10; m++)
   {
    ...
    finalall = finaloddall[$newodd] + finalevenall[$ijob];    
//  finalall = finaloddall[$newodd];
//  finalall = finalevenall[$ijob]; 
